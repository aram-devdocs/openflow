/**
 * Nav primitive utility function tests
 *
...
/**
 * Re-implement the Nav utility functions for testing
...
function extractSpacingClasses(props: NavSpacingProps): string[] {
  const classes: string[] = [];
...
describe('primitives/Nav - Utility Functions', () => {
  ...
  describe('Nav component requirements', () => {
    it('aria-label is optional', () => {
      // Nav does not require aria-label (unlike Section)
      // but it is recommended when multiple navs exist
      const optionalProps = ['aria-label'];
      expect(optionalProps).toContain('aria-label');
    });

    it('renders as a nav element', () => {
      // Nav always renders as <nav>
      const element = 'nav';
      expect(element).toBe('nav');
    });

    it('is a landmark region', () => {
      // Nav elements are exposed as landmark regions
      const landmarkRole = 'navigation';
      expect(landmarkRole).toBe('navigation');
    });

    it('supports multiple instances with unique labels', () => {
      // Best practice: unique aria-labels for multiple navs
      const navLabels = ['Main navigation', 'Footer navigation', 'Breadcrumb'];
  ...
  describe('Nav spacing scenarios', () => {
    it('generates classes for main navigation pattern', () => {
      const classes = extractSpacingClasses({
        px: { base: '4', md: '6', lg: '8' },
        py: '4',
      });
    ...
    it('generates classes for sidebar navigation pattern', () => {
      const classes = extractSpacingClasses({
        p: '4',
        gap: '2',
      });
    ...
    it('generates classes for breadcrumb pattern', () => {
      const classes = extractSpacingClasses({
        py: '2',
        px: { base: '4', md: '0' },
      });
    ...
    it('generates classes for footer navigation pattern', () => {
      const classes = extractSpacingClasses({
        p: { base: '6', md: '8' },
        gap: { base: '4', md: '6' },
      });
    ...
    it('generates classes for mobile navigation overlay', () => {
      const classes = extractSpacingClasses({
        p: '6',
        pt: '16',
        gap: '4',
  ...
  describe('Nav use cases', () => {
    it('is appropriate for main site navigation', () => {
      const validUseCases = ['main navigation'];
      expect(validUseCases).toContain('main navigation');
    });

    it('is appropriate for sidebar navigation', () => {
      const validUseCases = ['sidebar navigation'];
      expect(validUseCases).toContain('sidebar navigation');
    });

    it('is appropriate for footer links', () => {
      const validUseCases = ['footer links'];
      expect(validUseCases).toContain('footer links');
    });

    it('is appropriate for breadcrumbs', () => {
      const validUseCases = ['breadcrumbs'];
      expect(validUseCases).toContain('breadcrumbs');
    });

    it('is appropriate for pagination', () => {
      const validUseCases = ['pagination'];
      expect(validUseCases).toContain('pagination');
    });

    it('is appropriate for table of contents', () => {
      const validUseCases = ['table of contents'];
      expect(validUseCases).toContain('table of contents');
    });
  });

  describe('Nav vs Section comparison', () => {
    it('Nav is for navigation links', () => {
      const navCharacteristics = [
        'navigation links',
        'landmark region',
  ...
  describe('Accessibility requirements', () => {
    it('should use unique aria-label when multiple navs exist', () => {
      // When a page has multiple navs, each should have a unique aria-label
      const multipleNavLabels = ['Main navigation', 'Sidebar navigation', 'Footer navigation'];
      const uniqueLabels = new Set(multipleNavLabels);
      expect(uniqueLabels.size).toBe(multipleNavLabels.length);
    });

    it('should contain navigation links', () => {
      // Nav should primarily contain links
      const expectedContent = ['links', 'lists of links'];
      expect(expectedContent).toContain('links');
    });

    it('should have proper landmark role', () => {
      // nav elements have implicit role="navigation"
      const implicitRole = 'navigation';
      expect(implicitRole).toBe('navigation');
    });
  });
});
