/**
 * Section primitive utility function tests
 *
...
/**
 * Re-implement the Section utility functions for testing
 * (mirrors the logic in Section.tsx)
 */
...
function extractSpacingClasses(props: SectionSpacingProps): string[] {
  const classes: string[] = [];
...
describe('primitives/Section - Utility Functions', () => {
  ...
  describe('Section component requirements', () => {
    it('aria-label is a required prop (TypeScript enforcement)', () => {
      // This test documents the requirement - actual type checking is done by TypeScript
      // The SectionProps interface requires 'aria-label' to be a non-optional string
      const requiredProps = ['aria-label'];
      expect(requiredProps).toContain('aria-label');
    });

    it('renders as a section element', () => {
      // This test documents that Section always renders as <section>
      // Unlike Box, Section is not polymorphic
  ...
  describe('Section spacing scenarios', () => {
    it('generates classes for full-width section pattern', () => {
      const classes = extractSpacingClasses({
    ...
    it('generates classes for hero section pattern', () => {
      const classes = extractSpacingClasses({
      ...
      expect(classes).toContain('lg:py-24');
      expect(classes).toContain('px-4');
