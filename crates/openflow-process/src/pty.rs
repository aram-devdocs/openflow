...
mod tests {
    ...
    fn test_pty_config_new() {
        let config = PtyConfig::new("echo", &["hello", "world"]);
    ...
    fn test_pty_config_builder() {
        let config = PtyConfig::new("ls", &["-la"])
            .with_cwd("/tmp")
            .with_size(120, 40);
    ...
    fn test_pty_error_display() {
        let err = PtyError::NotFound("test".to_string());
        assert!(err.to_string().contains("PTY not found"));

        let err = PtyError::AlreadyExists("test".to_string());
        assert!(err.to_string().contains("PTY already exists"));

        let err = PtyError::LockPoisoned;
