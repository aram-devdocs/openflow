...
function generateCommandMap(endpoints: Endpoint[]): string {
  ...
  lines.push(`    throw new Error(\`Unknown command: \${command}. Ensure it's defined in openflow-contracts.\`);`);
  lines.push(`  }`);
  lines.push(``);
  ...
  lines.push(`    } else {`);
  lines.push(`      console.warn(\`Missing path parameter '\${param}' for command '\${command}'\`);`);
  lines.push(`    }`);
  lines.push(`  }`);
  lines.push(``);
  ...
  lines.push(`    const value = args[param];`);
  lines.push(`    if (value !== undefined && value !== null) {`);
  lines.push(`      queryParts.push(\`\${encodeURIComponent(param)}=\${encodeURIComponent(String(value))}\`);`);
  lines.push(`    }`);
  lines.push(`  }`);
  lines.push(``);
  ...
  const sortedMethods = Array.from(methodCounts.entries()).sort((a, b) =>
