...
// =============================================================================
// Event Unwrapping Utilities
// =============================================================================

/**
 * Wrapped event format from TauriBroadcaster.
 * Events emitted via the broadcaster include metadata wrapper.
 */
interface WrappedEvent<T = unknown> {
  process_id?: string;
  session_id?: string;
  event?: T;
  timestamp?: string;
...
/**
 * Unwrap a potentially wrapped event payload.
 *
 * The TauriBroadcaster wraps events with metadata (process_id, timestamp, etc).
 * This function extracts the inner event or returns the raw event if not wrapped.
 *
 * Handles both formats:
 * - Raw: `{ type: "assistant", ... }`
 * - Wrapped: `{ process_id: "...", event: { type: "assistant", ... }, timestamp: "..." }`
 *
 * @param payload - The event payload (raw or wrapped)
 * @returns The unwrapped event data
 *
 * @example
 * ```ts
 * const unsubscribe = subscribe('claude-event-123', (rawPayload) => {
 *   const event = unwrapEvent(rawPayload);
 *   console.log('Event type:', event.type);
 * });
 * ```
 */
export function unwrapEvent<T = unknown>(payload: unknown): T {
  if (!payload || typeof payload !== 'object') {
    return payload as T;
  }

  const p = payload as WrappedEvent<T>;

  // Check if this is a wrapped format (has 'event' field with an object)
  if (p.event && typeof p.event === 'object') {
    logger.debug('Unwrapping event', {
      hasProcessId: !!p.process_id,
      hasSessionId: !!p.session_id,
      hasTimestamp: !!p.timestamp,
    });
    return p.event;
  }

  // Return as-is (raw format)
  return payload as T;
}

/**
 * Check if an event payload is in wrapped format.
 *
 * @param payload - The event payload
 * @returns true if the payload is wrapped with metadata
 */
export function isWrappedEvent(payload: unknown): payload is WrappedEvent {
  if (!payload || typeof payload !== 'object') {
    return false;
  }
  const p = payload as WrappedEvent;
  return !!(p.event && typeof p.event === 'object');
}

/**
 * Unwrap a Claude/Agent event specifically.
 *
 * Claude events can come in two formats:
 * - Raw: `{ type: "system"|"assistant"|"user"|"result", ... }`
 * - Wrapped: `{ process_id: "...", event: {...}, timestamp: "..." }`
 *
 * @param payload - The event payload
 * @returns The Claude event or null if invalid
 */
export function unwrapClaudeEvent<T extends { type: string }>(payload: unknown): T | null {
  if (!payload || typeof payload !== 'object') {
    return null;
  }
  ...
  // Check for raw format (direct type field)
  ...
  logger.debug('Unable to unwrap claude event', { hasType: 'type' in p, hasEvent: 'event' in p });
  return null;
}
