...
export async function createChat(request: CreateChatRequest): Promise<Chat> {
  ...
  try {
    ...
    logger.info('create_chat completed', { id: (result as unknown as Record<string, unknown>).id });
...
export async function getChat(id: string): Promise<ChatWithMessages> {
  ...
  try {
    ...
    logger.info('get_chat completed', { id: (result as unknown as Record<string, unknown>).id });
 ...
 * @command list_chats
 * @param taskId - Query parameter: taskId (optional)
 * @returns Promise resolving to Chat[]
 * @throws Error if validation or query fails
 */
export async function listChats(taskId?: string): Promise<Chat[]> {
  ...
  try {
    const result = await invoke<Chat[]>('list_chats', { taskId: taskId });
 ...
 *
 * @endpoint GET /api/chats/by-project
...
export async function listChatsByProject(projectId?: string): Promise<Chat[]> {
  logger.debug('Calling list_chats_by_project');

  try {
    const result = await invoke<Chat[]>('list_chats_by_project', { projectId: projectId });
...
export async function listStandaloneChats(projectId?: string): Promise<Chat[]> {
  ...
  try {
    const result = await invoke<Chat[]>('list_standalone_chats', { projectId: projectId });
...
export async function updateChat(id: string, request: UpdateChatRequest): Promise<Chat> {
  ...
  try {
    ...
    logger.info('update_chat completed', { id: (result as unknown as Record<string, unknown>).id });
