...
import type { AuthStatusResponse, CliInstalledResponse, CreatePullRequestRequest, PullRequestResult } from '@openflow/generated';
...
export async function checkGhAuthStatus(): Promise<AuthStatusResponse> {
  ...
  try {
    ...
    logger.info('check_gh_auth_status completed', { id: (result as unknown as Record<string, unknown>).id });
...
export async function checkGhCliInstalled(): Promise<CliInstalledResponse> {
  ...
  try {
    ...
    logger.info('check_gh_cli_installed completed', { id: (result as unknown as Record<string, unknown>).id });
