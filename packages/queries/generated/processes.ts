...
export async function getProcess(id: string): Promise<ExecutionProcess> {
  ...
  try {
    ...
    logger.info('get_process completed', { id: (result as unknown as Record<string, unknown>).id });
 ...
 * @param id - Path parameter: id
 * @returns Promise resolving to ProcessSnapshot
 * @throws Error if validation or query fails
 */
export async function getProcessSnapshot(id: string): Promise<ProcessSnapshot> {
  logger.debug('Calling get_process_snapshot', { id });
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error('get_process_snapshot failed', { error: errorMessage });
    throw error;
  }
}
...
export async function listProcesses(chatId?: string): Promise<ExecutionProcess[]> {
  ...
  try {
    const result = await invoke<ExecutionProcess[]>('list_processes', { chatId: chatId });
